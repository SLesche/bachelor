...
The model estimates parameters defining the group-level distribution from which individual-level parameters are then drawn. Drift rates $\delta_{jk}$ of individual $j$ in condition $k$, for example, are assumed to follow the distribution $\delta_{jk} \sim N(\mu_{\delta k}, \sigma_{\delta k})$. 

(ref:method-display-model-spec-caption) Model Specification
...
prior_table <- data.table(
  ...
  Prior = c(
    "$\\mu_{\\delta} \\sim Cauchy(0, 5)$", "$\\sigma_{\\delta} \\sim Normal^{+}(0, 0.3)$",  "$\\alpha \\sim Normal^{+}(1.5, 1)$", "$\\sigma_{\\delta} \\sim Normal^{+}(0, 0.3)$", "$\\tau \\sim Normal^{+}(0.15, 0.1)$", "$\\sigma_{\\delta} \\sim Normal^{+}(0, 0.3)$"
  )
)
