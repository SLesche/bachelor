...
Effects of RSI condition and accuracy of the previous NoGo trial and their interaction on mean response times were then examined using a two-way repeated measures ANOVA. Post-error accuracy (PEA) was examined in a similar manner, comparing mean accuracy following a correct NoGo trial to mean accuracy of trials following an _error trial_.

## Bayesian hierarchical drift-diffusion model
The diffusion model was estimated using a Bayesian hierarchical approach implemented in the `brms` package [@R-brms_a]. Hierarchical approaches to the DDM allow simultaneous estimation of parameter influences on both a population-level aswell as on a subject-level [@vandekerckhove2011; @lee2011; @gelman2006], reducing the number of samples required to estimate model parameters reliably [@wiecki2013; @ratcliff2015]. Bayesian estimation allows hierarchical extensions of the model otherwise not feasible in frequentist approaches using maximum likelihood estimation [@vandekerckhove2011]. A further benefit of the bayesian approach towards DDM is the ability to fully infer posterior distributions and thus allow a more intuitive quantification of uncertainty [@kruschke2010].

## Model specification
A full description of model specification will be found in [Appendix B](##model-specification).
The model assumed fixed effects of the factors RSI (short - long) and accuracy of the previous NoGo-trial (C+1 - E+1) as well as participant-level random slopes of RSI. Intercepts in model specification were fixed to zero in order to estimate group-level parameters for each combination of factors instead of deviations from a baseline, easing specification of priors. **here link to tutorial**  **explain this better** 
Priors were specified in a weakly informative manner (see Table \@ref(tab-method-prior-table)).
...
prior_table <- data.table(
  ...
  prior = c(
    "\\delta \\sim Test", "Test", "Test", "Test"
  )
)

apa_table(prior_table, caption = "Lol", note = "Test")
```

Let $\mathbf{Y_{(pcaj)}}$ denote a response vector of the decision and response time $(X_{(pcaj)}, T_{(pcaj)})$ for the _p_ th participant, in the _j_ th trial in  condition _c_ (0,1) with previous accuracy _a_ (0,1). The data is assumed to follow a Wiener distribution,
$$\mathbf{Y_{(pcaj)}} \sim Wiener(\alpha_{(pcaj)}, \beta_{(pcaj)}, \tau_{(pcaj)}, \delta_{(pcaj)})$$
with the four model parameters boundary separation $\alpha$, bias $\beta$, non-decision time $\tau$ and drift-rate $\delta$. 
The index notation suggests that drift rates can differ across participants ($p$), conditions ($c$), previous accuracy ($a$) as well as across trials ($j$). This model was constrained by treating all trials for the same participant with the equal previous accuracy **in the same RSI? ** as identical. At the participant level, three parameters $\alpha$, $\tau$ and $\delta$ were allowed to vary between participants, with $\beta$ being fixed to 0.5. This was done to reduce model complexity, as there is no reason to assume a bias towards any decision. **maybe have it depend on map condition, or do a prior test here?**
...
Formulas
$$\delta_{pcaj} = \theta_{RSI(i)} + \beta_{2}A{j} + \beta_{3}A_jC_j + $$
<!-- maybe use this: equatiomatic::extract_eq(lme4::lmer(rt ~ 0 + rsi*error_factor + (0 + rsi|id), data = data_classic)) -->
**Give overview of formula and prior specification (informative, or no?)**
<!-- $$\delta_{(ij)} = \nu_{(i)} + \epsilon_{(ij)}$$ -->
<!-- $$\epsilon_{(ij)} \sim N(0, \eta^2_{\epsilon})$$ -->
<!-- $$\nu_{(p)} \sim N(\mu_{\nu}, \sigma^2_{\nu})$$ -->
<!-- Pros Hierarchical -->
<!-- - allow random and fixed effects -->
<!-- - take into account the shared variability between persons (population level) -->
<!-- - allow estimation of fixed effects of condition parameters (rsi, error) without estimating individual level parameters -->
<!-- - this lowers the sample size required -->
<!-- - allow group and subject parameters to be estimated at the same time but on different hierarchical levels -->

<!-- Pros Bayesian -->
<!-- - incorporating prior beliefs -->
<!-- - comparative hypothesis testing -->
<!-- - allow inference of full posterior distribution -> better treatment of uncertainty rather than the frequentist 'most likely value' approach -->
<!-- - intuitive treatment of uncertainty -->
