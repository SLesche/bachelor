...
### Behavioral analysis
Only errors in NoGo trials were considered _error trials_ relevant for analysis, because very few errors of commission occurred in Go trials. We classified Go trials based on the accuracy of the previous or following NoGo trial. Go trials following a NoGo-error are defined as trials _E+1_, Go trials preceding NoGo trials were defined as _E-1_ or _C-1_ depending on the accuracy of that NoGo trial. Go-trials following a correct NoGo trial do not receive a label and are excluded from analysis. These trials follow an inhibited response. The time at which participants make the decision not to respond and the onset of the next stimulus is longer than in most other trials. The effective RSI is therefore longer, since it is not equal to the predefined interval in the respective block. In order to have a homogeneous sample of _post-correct_ trials, we therefore excluded all trials following correct NoGo trials from being defined as _post-correct_. Only Go trials following correct Go trials are defined as _Corr+1_.
<!-- no C+1 for post-correct because confusing -->

To quantify the impact of an error, we used models testing the classical approach to PES [@rabbitt1979]. It defines PES as the difference in mean response time between trials following an error trial and trials following a correct Go trial.
$$\Delta_{PES} = \theta_{E+1} - \theta_{Corr+1}$$ 
We also used models testing the robust approach to PES [@dutilh2012how]. The robust approach defines PES as the difference in mean response time between trials following an error trial and trials that follow a correct (Go) trial and preceed an incorrect (NoGo) trial.
$$ \Delta_{PES} = \theta_{E+1} - \theta_{E-1}$$

When investigating the specific impact of an error-response on the following trial we will conduct our main analyses using the robust definition of PES to account for global shifts in performance. Parameters and behavioral measures in trials following an error will be compared to trials preceding an error to ensure that post-error trials and trials used to estimate a baseline have the same origin in the dataset. This prevents biased sampling of post-error trials depending on global performance shifts. While the robust approach is able to combat global shifts in performance, it is especially vulnerable to local shifts in performance, such as _pre-error speeding_ [@pfister2022]. In order to obtain a more complete picture of the data, we will inspect results using both the classical and robust approach and report differences between the two methods whenever they occur.

<!-- ^[We also conducted all behavioral and DDM analysis with the classical measure of PES [@dutilh2012how]. Results did not differ significantly, see Appendix C for  details on global performance shifts and model results.] -->
...
### Model analysis
We will attempt to run 2 different hierarchical models, testing post-error effects in both the classical and robust approach. All models run `r model_setup_values$n_chains` chains for `r model_setup_values$n_iter` iterations each, with `r model_setup_values$n_warmup` iterations per chain being used as a warmup to adapt the sampler. The final analysis is based on `r (model_setup_values$n_iter - model_setup_values$n_warmup)*model_setup_values$n_chains` iterations. Treedepth was set to `r model_setup_values$max_depth` and adapt_delta was set to `r model_setup_values$adapt_delta`. Following checks for model convergence, we will draw posterior samples and assess the fit of the model to our experimental data. We will compare posterior distributions of group-level parameters between levels of the conditions $k$ to investigate the effects of errors and RSI on diffusion model parameters.
