...
with the four model parameters boundary separation $\alpha$, bias $\beta$, non-decision time $\tau$ and drift-rate $\delta$. Responses $X_{(ij)}$ can take the values $X_{(ij)} = \{0,1\}$. $X_{(ij)} = 0$ represents the decision that the stimulus shows the letter _X_, corresponding to the lower response boundary. $X_{(ij)} = 1$ represents the respective decision that the stimulus shows the letter _Y_. Response time (in $s$) can take on any positive value $T_{(ij)} \in (0, 0.8]$. A response $\mathbf{Y_{(ij)}}$ of participant $i$ on trial $j$ is further influenced by the trial parameters $RSI_{j}$ (short - long), previous accuracy $ACC_{j}$(C+1 - E+1) and stimulus type $STIM_{j}$ ("X" - "Y").

The parameters drift rate $\delta$, non-decision time $\tau$ and boundary separation $\alpha$ were allowed to vary between participants and trials. For these three parameters, the model estimated fixed effects of the factors RSI (short - long), accuracy of the previous NoGo-trial (C+1 - E+1) and their interaction as well as participant-level random slopes for the factor RSI. All intercepts were fixed to zero in order to estimate group-level parameters for each combination of factors instead of deviations from a baseline, easing specification of priors [@singmann2017intro]. Drift rate was further allowed to vary depending on the true status of the presented stimulus, accounting for differences in drift direction depending on the presented stimulus. Priors for the regression weights of fixed effects were specified in a weakly informative manner (see Table \@ref(tab:method-spec-table)). `brms`-default priors were used for random effects. Bias $\beta$ was set to 0.5 for all participants and trials, as there was no reason to assume any a-priori bias towards one response alternative ("X" - "Y") ^[This assumption was confirmed by exploratory analysis with the bias parameter being allowed to vary freely between participants and response-mapping instructions. The bias parameter was estimated at 0.5 by the model with low individual-level variance.]. 
Correlations of random effects of each parameter were only estimated between conditions, not for individual-level deviations across parameters. None of the parameters were transformed before analysis, easing prior specification and model interpretation [@singmann2017intro]. **explain this better** **provide proof for bias estimation** **maybe put in supplementary information link (code)**
...
prior_table <- data.table(
  ...
  Formula = c(
    "$\\delta \\sim 0 + rsi:acc:stim + (0 + rsi|id)$",
    "$\\alpha \\sim 0 + rsi:acc + (0 + rsi|id)$",
    "$\\tau \\sim 0 + rsi:acc + (0 + rsi|id)$"
  )
)
# maybe put estimation formula in here?
apa_table(prior_table, caption = "Specification for diffusion model parameters", note = "Boundary separation and non-decision priors received a lower bound of 0. Default priors were used for group-level random effects. Formula represents lme4-like model syntax used for specification. 'acc' represents the accuracy of the previous trial. ", escape = FALSE)
```
