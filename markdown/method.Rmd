...
**remove first 10 trials for each participant of the first two blocks?**
...
Correlations of random effects of each parameter were only estimated between conditions, not across parameters. None of the parameters were transformed before analysis, easing prior specification and model interpretation [@singmann2017intro]. **explain this better** **provide proof for bias estimation** **maybe put in supplementary information link (code)**
...
apa_table(prior_table, caption = "Prior specification for diffusion model parameters", note = "Boundary separation and non-decision priors received a lower bound of 0. Default priors were used for group-level random effects.", escape = FALSE)
```
