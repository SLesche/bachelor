...
Mean reaction times $\overline{RT}$ in Go trials following an _error trial_ $E+1$ are compared to mean reaction times following a correct response to a NoGo trial $C+1$. Effects of RSI condition and accuracy of the previous NoGo trial and their interaction were then examined using a 2x2 ANOVA. Post-error accuracy (PEA) was examined in a similar manner, comparing mean accuracy following a correct NoGo trial to mean accuracy of trials following an _error trial_.

## Diffusion model specification
**Describe procedure for diffusion model**
The diffusion model was estimated using a Bayesian hierarchical approach implemented in the `brms` package [@R-brms_a]. **talk about why this good. Why Bayesian good, why hierarchical good? Lower trial_n required usw.** 
