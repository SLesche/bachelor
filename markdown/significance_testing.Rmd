...
```{r significance-testing-pre-error-speeding}
mean_data_rsi_pre <- data %>% 
  filter(location == "E-1" | location == "C-1") %>% 
...
aov_rt_pre_rsi <- aov_car(
  formula = rt_mean ~ rsi*location + Error(id/rsi*location),
  data = mean_data_rsi_pre,
  anova_table = list(es = c("ges","pes"))
)
```
