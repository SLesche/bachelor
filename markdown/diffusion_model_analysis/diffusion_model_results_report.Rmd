...
# Reading in data
fit_base_model <- rio::import("./markdown/diffusion_model_runs/cluster/output/base_model.rda")
pred_base_model <- rio::import("./markdown/diffusion_model_runs/cluster/output/predictions/pred_base_model.rda")
data_base_model <- fit_base_model$data

combined_base_model <- combine_data(data_base_model, pred_base_model)
```

```{r diffusion-results-ep1-bad-pred-prep, cache = TRUE}
vector_ep1 <- combined_base_model %>% 
    filter(rsi == "short", error_factor == "E+1") %>% 
