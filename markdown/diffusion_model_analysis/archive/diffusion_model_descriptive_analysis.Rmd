...
plot_params_classic <- function(data){
  data %>% 
    ggplot(
      aes(
        x = location,
...
plot_long_rsi_classic <- function(data){
  data %>% 
    filter(rsi == "long") %>% 
    ggplot(
      aes(
...
fit_hierarch_robust <- rio::import("./markdown/diffusion_model_runs/cluster/output/hierarch_model_long_robust.rda")
```

```{r ddm-descriptive-counting-location}
model_robust_count <- fit_hierarch_robust$data %>% 
