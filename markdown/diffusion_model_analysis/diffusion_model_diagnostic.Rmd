...
data_model <- rio::import("./markdown/models/classic_standard_rsi_err_2023-02-15.rda")
```
...
sum(subset(nuts_params(data_model), Parameter=="divergent__")$Value)
```

```{r visual-par-inspection}
pars <- variables(data_model)
pars_sel <- c(
  sample(pars[1:(length(pars)/2)], 3),
  sample(pars[-(1:(length(pars)/2))], 3)
  )

plot(data_model, variable = pars_sel, N = 6, 
     ask = FALSE, exact_match = TRUE, newpage = TRUE, plot = TRUE)
```

```{r divergent problems}
pairs(data_model$fit, variable = pars_sel, condition = "divergent__")

```

```{r parameter correlations}
posterior <- as.mcmc(data_model, combine_chains = TRUE)
