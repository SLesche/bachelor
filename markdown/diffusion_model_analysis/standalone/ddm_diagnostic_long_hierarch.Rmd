...
generate_med_rts_pred <- function(combined_data, raw_data, group_id = FALSE){
  if (group_id == FALSE){
    pred_rts = combined_data %>% 
      group_by(error_factor) %>%  # select grouping vars
      ...
      ungroup() %>% 
      gather("key", "value", -error_factor, -stimulus) %>% # remove grouping vars
    ...
    pred_med = combined_data %>% 
      group_by(id, error_factor) %>%  # select grouping vars
      summarise_at(.vars = vars(starts_with("V")), 
                   funs(prob.upper = mean(. > 0),
                        medrt.lower = median(abs(.[. < 0]) ),
                        medrt.upper = median(.[. > 0] )
      ...
      ungroup() %>% 
      gather("key", "value", -id, -error_factor) %>% # remove grouping vars
      separate("key", c("rep", "measure"), sep = "_") %>% 
      spread(measure, value) %>% 
      group_by(id, error_factor) %>% # select grouping vars
      summarise_at(.vars = vars(prob.upper, medrt.lower, medrt.upper), 
...
generate_plot_median_lower <- function(data){
  labels <- data %>% 
    mutate(labels = paste0(error_factor, "_", stimulus)) %>% 
    pull(labels)
  labels_upper <- labels[which(str_detect(labels, "_1$"))]
  labels_lower <- labels[which(str_detect(labels, "_0$"))]
...
return_vector_of_values <- function(data, my_error_factor, my_stimulus){
  vector <- data %>% 
    filter(error_factor == my_error_factor) %>% 
...
compare_plot <- function(dataframe, vector, my_error_factor){
  data = dataframe %>% 
    filter(error_factor == my_error_factor) %>% 
...
compare_all_conditions <- function(data){
  ...
  for (i in seq_along(factor_combinations)) {
    vec_error_factor = factor_combinations[i]
...
```{r data}
# Investigate cluster
fit_wiener_robust <- rio::import("./markdown/diffusion_model_runs/cluster/output/hierarch_model_long_robust.rda")
pred_robust <- rio::import("./markdown/diffusion_model_runs/cluster/output/predictions/pred_hierarch_robust.rda")
data_robust <- fit_wiener_robust$data
```
...
```{r ppc}
neg_data <- data %>% 
  mutate(rt = ifelse(acc == 0, rt*-1, rt))

hist(neg_data$rt)

# separate into conditions
pre_error_rt <- neg_data %>% 
  filter(error_factor == "pre_error") %>% 
  pull(rt)

post_error_rt <- neg_data %>% 
  filter(error_factor == "post_error") %>% 
  pull(rt)

# Turn into long
pred_pre_error_rt <- combined_data %>% 
  filter(error_factor == "pre_error") %>% 
  select(starts_with("V")) %>% 
  as.matrix() %>% 
  c()

pred_post_error_rt <- combined_data %>% 
  filter(error_factor == "post_error") %>% 
  select(starts_with("V")) %>% 
  as.matrix() %>% 
  c()

robust_pre_error_plot <- ggplot()+
  geom_density(
    aes(
      x = abs(pre_error_rt)
    ),
...
save(visual_fit, file = "./markdown/diffusion_model_analysis/posterior_predictive_fits/plots_base_model.rda")
```
