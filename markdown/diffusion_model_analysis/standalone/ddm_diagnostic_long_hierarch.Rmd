...
generate_med_rts_pred <- function(combined_data, raw_data, group_id = FALSE){
  if (group_id == FALSE){
    pred_rts = combined_data %>% 
      group_by(error_factor, stimulus) %>%  # select grouping vars
    ...
    pred_med = combined_data %>% 
      group_by(id, error_factor, stimulus) %>%  # select grouping vars
      summarise_at(.vars = vars(starts_with("V")), 
                   funs(prob.upper = mean(. > 0),
                        medrt.lower = median(abs(.[. < 0]) ),
                        medrt.upper = median(.[. > 0] )
      ...
      ungroup() %>% 
      gather("key", "value", -id, -error_factor, -stimulus) %>% # remove grouping vars
      separate("key", c("rep", "measure"), sep = "_") %>% 
      spread(measure, value) %>% 
      group_by(id, error_factor, stimulus) %>% # select grouping vars
      summarise_at(.vars = vars(prob.upper, medrt.lower, medrt.upper), 
...
summarize_med_rt <- function(object, group_id = FALSE){
  if (group_id == FALSE){
    summary = object %>% 
      group_by(error_factor, stimulus) %>% 
      summarise_if(is.numeric, mean, na.rm = TRUE) %>% 
      ungroup()
  } else {
    summary = object %>% 
      group_by(id, error_factor, stimulus) %>% 
      summarise_if(is.numeric, mean, na.rm = TRUE) %>% 
      ungroup()
  }
}
...
generate_plot_resp_prob <- function(data){
  ...
  plot <- ggplot(data, aes(x = error_factor:stimulus)) +
...
generate_plot_median_upper <- function(data){
  ...
  plot <- ggplot(data %>% filter(stimulus == 1), aes(x = error_factor)) +
...
return_vector_of_values <- function(data, my_error_factor, my_stimulus){
  vector <- data %>% 
    filter(error_factor == my_error_factor, stimulus == my_stimulus) %>% 
...
compare_plot <- function(dataframe, vector, my_error_factor, my_stimulus){
  data = dataframe %>% 
    filter(error_factor == my_error_factor, stimulus == my_stimulus) %>% 
...
grid.arrange(grobs = robust_visual_fit)
