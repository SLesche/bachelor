...
working_robust <- data_robust %>% 
  ...
  mutate(
    error_factor = str_remove(error_factor, "_error$")
  ) %>% 
  separate(
    error_factor,
...
summary_robust <- working_robust %>% 
  mutate(upper_copy = upper, lower_copy = lower) %>% 
  mutate(lower = ifelse(param == "drift" & stimulus == 0, upper_copy, lower),
         upper = ifelse(param == "drift" & stimulus == 0, lower_copy, upper)) %>% 
  mutate(mean_corrected = abs(mean), lower_corrected = abs(lower), upper_corrected = abs(upper)) %>% 
  group_by(param, error_factor, stimulus) %>% 
  summarize(
