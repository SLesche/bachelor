...
In the robust model, we observed a significant decrease of drift rate in post-error trials compared to pre-error levels in the long RSI condition `r print_cohen_num(cohens_d_robust_drift_mean_d)`. Additionally, boundary separation decreased `r print_cohen_num(cohens_d_robust_bs_mean_d)`, non-decision time decreased `r print_cohen_num(cohens_d_robust_ndt_mean_d)` and starting point bias was reduced `r print_cohen_num(cohens_d_robust_bias_mean_d)`. In the classical model, we observed the same decreases in drift rate `r print_cohen_num(cohens_d_classic_drift_mean_d)` and boundary separation `r print_cohen_num(cohens_d_classic_bs_mean_d)` but observed increases in non-decision time `r print_cohen_num(cohens_d_classic_ndt_mean_d)` and starting point `r print_cohen_num(cohens_d_classic_bias_mean_d)`.
