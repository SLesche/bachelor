...
The best-fitting model specification was used to estimate parameters in all 3 models and was then subjected to various model diagnostic tests. In the robust model, there were `r n_divergent(fit_robust)` divergent transitions, $\widehat{R} > 1.01$ or other signs of non-convergence [@gelman1992]. We visually inspected whether the four chains mixed well for a number of randomly selected parameters, observing no signs of misbehavior in the chains. We sampled the posterior parameter distribution 200 times, obtained predicted response time distributions from the sampled parameters and visually inspected them for appropriate model fit. 

We observed `r n_divergent(fit_classic)` divergent transitions when fitting the classical model to our data. Inspections of $\widehat{R}$, bulk- and tail effective sample size and trace plots of the MCMC chains showed no signs of issues in the model. We will refrain from interpreting the parameters of the classical model in this study, but report the results in Appendix C. Main effects are of the same direction in both models.
...
Seeing as significance tests are based on a minimum of 8000 iterations per condition, all differences presented here are highly significant. We choose to only report effect sizes and omit exact t-test results. 

We observed a significant decrease of drift rate in post-error trials compared to pre-error levels in the long RSI condition `r print_cohen(cohens_d_robust_drift)`. Additionally, boundary separation decreased `r print_cohen(cohens_d_robust_bs)`, non-decision time increased `r print_cohen(cohens_d_robust_ndt)` and starting point bias was reduced `r print_cohen(cohens_d_robust_bias)`^[Starting point bias was increased in the classical account.].

```{r results-plots-robust, fig.cap='Robust approach to PES. Group-level parameter estimates of mean of DDM parameters. Error bars represent 95% CrI.'}
combined_robust_plot
```

We also compared trials prior to errors (E-1) to those preceding correct responses (C-1) to investigate _pre-error speeding_. The hierarchical model did not converge properly when attempting to estimate the short RSI condition. Again, we discarded the data from the short RSI condition and tried refitting the model in the long RSI condition with a higher adapt-delta. `r n_divergent(fit_speed)` divergent transitions remained, indicating that the MCMC sampling algorithm was not able to fully explore the parameter space. $\widehat{R}$ values and MCMC-trace plots indicated that the chains mixed well.

In the behavioral analysis, we observed shorter response times prior to errors `r apa_print(aov_rt_pre_rsi, es = "pes", mse = FALSE)$full_result$location`, with the effect being more pronounced in the long RSI condition `r apa_print(aov_rt_pre_rsi, es = "pes", mse = FALSE)$full_result$rsi_location`.

In the long RSI condition we observed lower drift rates `r print_cohen(cohens_d_speed_drift)` and lower boundary separation in pre-error trials `r print_cohen(cohens_d_speed_bs)`. Non-decision time was higher `r print_cohen(cohens_d_speed_ndt)` and the starting point bias was increased `r print_cohen(cohens_d_speed_bias)`.

```{r results-plots-speed, fig.cap='Group-level parameter estimates of mean of DDM parameters investigating pre-error speeding. Error bars represent 95% CrI.'}
combined_speed_plot
```
