...
### Model Fit
The best-fitting model specification was used to estimate parameters in the full location dataset and was then subjected to various model diagnostic tests. There were no divergent transitions, $\hat{R} > 1.01$ or other signs of non-convergence [@gelman1992]. We visually inspected if the 4 chains mixed well for a few randomly selected parameters, observing no signs of misbehavior in the chains.
...
Because we estimated drift rate and bias depending on the presented stimulus, we obtained posterior distributions for each presented stimulus. We find main effects depending on the presented stimulus `r apa_print(t_test_robust_stimulus_drift)$statistic` and thus chose not to collapse posterior samples of both stimulus conditions into one as the resulting distribution is not normal.

We observed a significant decrease in drift rate in post-error trials compared to pre-error levels in the long RSI condition for both stimulus conditions `r apa_print(t_test_robust_drift_0)$statistic`, _d =_ `r apa_num(cohens_d_robust_drift_0$Cohens_d)`  and `r apa_print(t_test_robust_drift_1)$statistic`, _d =_ `r apa_num(cohens_d_robust_drift_1$Cohens_d)` respectively. Boundary separation decreased `r apa_print(t_test_robust_bs)$statistic`, _d =_ `r apa_num(cohens_d_robust_bs$Cohens_d)`, non-decision time increased `r apa_print(t_test_robust_ndt)$statistic`, _d =_ `r apa_num(cohens_d_robust_ndt$Cohens_d)` and responses were less biased towards the previously not shown response in both _previous stimulus_ conditions `r apa_print(t_test_robust_bias_X)$statistic`, _d =_ `r apa_num(cohens_d_robust_bias_X$Cohens_d)`  and `r apa_print(t_test_robust_bias_Y)$statistic`, _d =_ `r apa_num(cohens_d_robust_bias_Y$Cohens_d)` respectively. Graphical illustrations of parameter development following errors and correct NoGo responses can be found in Appendix D.
...
In the long RSI condition we observed lower drift rates `r apa_print(t_test_pre_long_drift_0)$estimate`, _d=_ `r apa_num(cohens_d_pre_long_drift_0$Cohens_d)` and lower boundary separation in pre-error trials `r apa_print(t_test_pre_long_bs)$estimate`, _d=_ `r apa_num(cohens_d_pre_long_bs$Cohens_d)`. Non-decision time was lower `r apa_print(t_test_pre_long_ndt)$estimate`, _d=_ `r apa_num(cohens_d_pre_long_ndt$Cohens_d)` and responses were more biased  with the decrease in boundary separation being responsible for faster response times.
