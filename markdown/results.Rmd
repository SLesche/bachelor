...
```
```{r results-source-ddm-prep, child = "markdown/diffusion_model_analysis/standalone/diffusion_model_prep.Rmd"}
```
...
### Behavioral analysis
All analyses were conducted using `r r_citations$r`. `r r_citations$pkgs`
After data cleaning, `r nrow(final_data)` trials from `r demo_output$n_subjects - n_outliers` participants remained. Participants falsely responded in `r descriptive_output$mean_error_rate_nogo`% of NoGo trials resulting in a median of `r average_n_location %>% filter(location == "E-1") %>% pull(median)` (SD = `r average_n_location %>% filter(location == "E-1") %>% pull(sd)`) trials prior to an error (E-1), and a median of `r average_n_location %>% filter(location == "E+1") %>% pull(median)` (SD = `r average_n_location %>% filter(location == "E+1") %>% pull(sd)`) trials following an error (E+1) per participant.

Using the robust approach, we observed highly significant main effects of RSI, `r apa_print(aov_rt_rsi_error_robust, es = "pes", mse = FALSE)$full_result$rsi` and error-condition, `r apa_print(aov_rt_rsi_error_robust, es = "pes", mse = FALSE)$full_result$error_code` on response time. Response times were slower in the short RSI condition and slower following an error. Critically, the interaction between RSI and error-condition was highly significant, `r apa_print(aov_rt_rsi_error_robust, es = "pes", mse = FALSE)$full_result$rsi_error_code`. A post-hoc comparison reveals an increased influence of error-condition in the short RSI condition, manifesting in increased PES. This pattern was also found using the classical definition of PES. However, we observed slight _post-error speeding_ following errors in the long RSI condition. Descriptive statistics can be found in Table \@ref(tab:appendix-tab-robust-desc) and \@ref(tab:appendix-tab-classic-desc).

Analysis of mean accuracy between the 2x2 conditions yielded similar patterns of significant main effects of RSI `r apa_print(aov_acc_rsi_error_robust, es = "pes", mse = FALSE)$full_result$rsi` and error-condition `r apa_print(aov_acc_rsi_error_robust, es = "pes", mse = FALSE)$full_result$error_code` and a significant interaction effect, `r apa_print(aov_acc_rsi_error_robust, es = "pes", mse = FALSE)$full_result$rsi_error_code`. Errors committed in the short RSI condition lead to a more significant decrease in accuracy on trials immediately following an error.

```{r results-include-behavioral-plots-robust, warning = FALSE, fig.cap="Robust approach to post-error effects. Mean accuracy and response times preceding and following error trials. Omissions were excluded from accuracy estimation. Error bars represent 95% confidence intervals."}
combined_robust_plot_behavioral
```

```{r results-include-behavioral-plots-classic, warning = FALSE, fig.cap="Classical approach to post-error effects. Mean accuracy and response times following error trials vs. following correct trials. Omissions were excluded from accuracy estimation. Error bars represent 95% confidence intervals."}
combined_classic_plot_behavioral
```
...
The previously described base model, which allows all parameters to be affected by RSI and error condition, was superior to all other models tested. Thus we retained the influence of both factors on all parameters for analysis. When restricting the influence of RSI on only one of the parameters, restricting its influence on the drift rate had the most detrimental effects. Constraining the bias and boundary separation to be equal across RSI conditions barely decreased predictive accuracy (see Table \@ref(tab:appendix-rsi-compare)). A similar pattern was found when restricting the impact of error-condition on only one of the parameters while allowing all other parameters to vary between pre- and post-error. Here, restricting the drift rate parameter again had the strongest negative impact on predictive accuracy (see Table \@ref(tab:appendix-err-compare)). When only allowing one of the four diffusion model parameters to vary between error-condition, drift rate proved to be the single most important parameter for good predictive accuracy (see Table \@ref(tab:appendix-only-compare)).

#### Model fit
First, we ran non-hierarchical models, tested them for convergence and obtained posterior predictive response time distributions. We then compared empirical response time distributions to the predicted response time distribution for each condition. This revealed a serious discrepancy between the predicted response time distribution and the empirical distribution in post-error trials in the short RSI condition (see Figure \@ref(fig:results-plot-ep1-bad-pred)). This also lead to issues when attempting to fit hierarchical models to data containing this condition. We therefore only fit hierarchical models to data in the long RSI condition.

The best-fitting model specification was used to estimate parameters in both the classic and robust model and was then subjected to various model diagnostic tests. In the robust model, there were `r n_divergent(fit_robust)` divergent transitions, $\widehat{R} > 1.01$ or other signs of non-convergence [@gelman1992]. We visually inspected whether the four chains mixed well for a number of randomly selected parameters, observing no signs of misbehavior in the chains. We sampled the posterior parameter distribution 200 times, obtained predicted response time distributions from the sampled parameters and visually inspected them for appropriate model fit. 

We observed `r n_divergent(fit_classic)` divergent transitions when fitting the classical model to our data. Inspections of $\widehat{R}$, bulk- and tail effective sample size and trace plots of the MCMC chains showed no signs of issues in the model. All parameters obtained from the classical model in this study should be taken with caution, as the number of divergent transitions indicate that the sampler was not able to fully explore the parameter space.
...
#### Model results
The final analysis of the robust model was based on a total of `r model_robust_count %>% filter(error_factor == "post_error") %>% pull(total)` post-error and `r model_robust_count %>% filter(error_factor == "pre_error") %>% pull(total)` pre-error trials with a median of `r model_robust_count %>% filter(error_factor == "post_error") %>% pull(median)` trials and `r model_robust_count %>% filter(error_factor == "pre_error") %>% pull(median)` trials per participant, respectively. The model estimated a total of 24 group-level parameters. We obtained posterior distributions of the model parameters and calculated difference scores between error conditions. This difference score describes the impact of an error on DDM parameters. We calculated Bayes Factors and effect sizes based on this distribution of differences. The Bayes factor was approximated by the ratio of prior and posterior density at the value zero [@dickey1971]. It indicates whether the null hypothesis of no effect between the two conditions has become more or less likely given the observed distribution of differences. A Bayes Factor $< 1$ indicates support in favor of the null hypothesis. Bayes Factors between 3 and 10 indicate moderate support in favor of the alternative hypothesis and a Bayes Factor $> 10$ indicates strong support in favor of the alternative hypothesis [@lee2014bayesian].

In the robust model, we observed a significant decrease of drift rate in post-error trials compared to pre-error levels in the long RSI condition (`r print_bf(bf_robust_drift)`, `r print_cohen_num(cohens_d_robust_drift_mean_d)`). Additionally, boundary separation decreased (`r print_bf(bf_robust_bs)`, `r print_cohen_num(cohens_d_robust_bs_mean_d)`), non-decision time decreased (`r print_bf(bf_robust_ndt)`, `r print_cohen_num(cohens_d_robust_ndt_mean_d)`) and starting point bias was reduced (`r print_bf(bf_robust_bias)`, `r print_cohen_num(cohens_d_robust_bias_mean_d)`). The Bayes Factors indicate low support for effects of errors on non-decision time and boundary separation. In the classical model, we observed the same decreases in drift rate (`r print_bf(bf_classic_drift)`, `r print_cohen_num(cohens_d_classic_drift_mean_d)` and boundary separation (`r print_bf(bf_classic_bs)`, `r print_cohen_num(cohens_d_classic_bs_mean_d)`) but observed increases in non-decision time (`r print_bf(bf_classic_ndt)`, `r print_cohen_num(cohens_d_classic_ndt_mean_d)`) and starting point (`r print_bf(bf_classic_bias)`, `r print_cohen_num(cohens_d_classic_bias_mean_d)`). Again, the Bayes Factors indicate low support for effects of errors on non-decision time and boundary separation. 

```{r results-plots-robust, fig.cap='Robust approach to PES. Group-level parameter estimates of mean DDM parameters. Error bars represent 95% CrI.'}
combined_robust_plot
```
