...
```{r data-prep-coding error types, cache = TRUE, dependson='data-prep-importing-data'}
...
error_data <- rt_data %>% 
  ...
  mutate(
    location = ifelse(
      lag(location, 2) == "E+0" & is.na(location) & trial_type == "go",
      "E+2",
      location
    )
  ) %>% 
  mutate(
    location = ifelse(
      lag(location, 2) == "C+0" & is.na(location) & trial_type == "go",
      "C+2",
      location
    )
  ) %>% 
