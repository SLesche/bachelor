...
```{r descriptive-analysis-classic pes, include = FALSE}
...
```{r descriptive-analysis-block comparison, include = FALSE, cache = TRUE, dependson='data'}
data %>% 
  group_by(rsi, block) %>% 
...
```{r descriptive-analysis-error rate without noresponse, include = FALSE}
data %>% 
  group_by(rsi, error_code) %>% 
  filter(!is.na(rt) & trial_type == "go" | trial_type == "nogo") %>% 
  summarize(
...
```{r descriptive-analysis-fast slow error, include = FALSE}
data %>% 
...
```{r descriptive-analysis-condition comparison, include = FALSE}
data %>% 
...
# put SD in there too
apa_table(data.frame("placeholder"), caption = "Placeholder Table")
```
