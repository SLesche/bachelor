...
proportion_omissions_location <- data %>% 
  group_by(rsi, location) %>% 
  count(is.na(rt)) %>% 
  pivot_wider(
    id_cols = c("rsi", "location"),
    values_from = n,
    names_from = `is.na(rt)`
  ) %>% 
  mutate(
...
```{r descriptive-analysis-classic-robust-plots}
behavioral_plot_basis_classic <- data %>% 
  group_by(rsi, classic_pes_type) %>% 
  filter(!is.na(rt) & trial_type == "go" | trial_type == "nogo") %>% 
   summarize(
    across(c("rt", "acc"),
  ...
  filter(!is.na(classic_pes_type)) %>% 
  mutate(rt_se = rt_sd/sqrt(rt_count), 
         acc_se = acc_sd/sqrt(acc_count))

behavioral_plot_basis_robust <- data %>% 
  group_by(rsi, error_code) %>% 
  filter(!is.na(rt) & trial_type == "go" | trial_type == "nogo") %>% 
   summarize(
    across(c("rt", "acc"),
  ...
  filter(!is.na(error_code) & error_code != 0) %>% 
  mutate(rt_se = rt_sd/sqrt(rt_count), 
         acc_se = acc_sd/sqrt(acc_count))

behavioral_plot_pes_classic_rt <- behavioral_plot_basis_classic %>% 
  ggplot(
    aes(
      x = factor(rsi),
      y = rt_mean,
      group = classic_pes_type,
      fill = factor(classic_pes_type)
    )
  )+
  geom_bar(position = position_dodge(width=0.9), stat = "identity")+
  geom_errorbar(
    aes(
      ymin = rt_mean - qnorm(0.975)*rt_se, 
      ymax = rt_mean + qnorm(0.975)*rt_se,
      width = 0.6
    ), 
    position=position_dodge(width=0.9)
  )+
  scale_x_discrete(
    labels = c("Short RSI", "Long RSI")
  )+
  scale_fill_manual(
    labels = c("post-correct", "post-error"),
...
behavioral_plot_pes_classic_acc <- behavioral_plot_basis_classic %>% 
  ggplot(
    aes(
      x = factor(rsi),
      y = acc_mean,
  ...
  geom_errorbar(
    aes(
      ymin = acc_mean - qnorm(0.975)*acc_se, 
      ymax = acc_mean + qnorm(0.975)*acc_se,
      width = 0.6
    ), 
    position=position_dodge(width=0.9)
  )+
...
behavioral_plot_pes_robust_acc <- behavioral_plot_basis_robust %>% 
  ggplot(
    aes(
      x = factor(rsi),
      y = acc_mean,
      group = error_code,
      fill = factor(error_code)
    )
  )+
  geom_bar(position = position_dodge(width=0.9), stat = "identity")+
  geom_errorbar(
...
behavioral_plot_pes_robust_rt <- behavioral_plot_basis_robust %>% 
  ggplot(
    aes(
      x = factor(rsi),
      y = rt_mean,
...
combined_robust_plot_behavioral <- ggarrange(behavioral_plot_pes_robust_rt, behavioral_plot_pes_robust_acc, labels = c("RT", "ACC"), common.legend = TRUE, legend = "right")
combined_classic_plot_behavioral <- ggarrange(behavioral_plot_pes_classic_rt, behavioral_plot_pes_classic_acc, labels = c("RT", "ACC"), common.legend = TRUE, legend = "right")
```

```{r descriptive-analysis-overview-robust, include=FALSE}
n_omissions_robust <- data %>% 
  count(error_code, rsi, response == "None") %>% 
  filter(error_code %in% c(-1, 1)) %>% 
  group_by(error_code, rsi) %>% 
  mutate(omission_rate = n / sum(n)) %>% 
  filter(`response == "None"` == TRUE) %>% 
  select(error_code, rsi, omission_rate)

table_overview_robust <- data %>% 
  filter(!is.na(rt) & trial_type == "go" | trial_type == "nogo") %>% 
  group_by(rsi, error_code) %>%
  summarize(
    across(
...
```{r descriptive-analysis-overview-classic, include=FALSE}
n_omissions_classic <- data %>% 
  count(classic_pes_type, rsi, response == "None") %>% 
  filter(!is.na(classic_pes_type)) %>% 
  group_by(classic_pes_type, rsi) %>% 
  mutate(omission_rate = n / sum(n)) %>% 
  filter(`response == "None"` == TRUE) %>% 
  select(classic_pes_type, rsi, omission_rate)

table_overview_classic <- data %>% 
  filter(!is.na(rt) & trial_type == "go" | trial_type == "nogo") %>% 
  group_by(rsi, classic_pes_type) %>%
  summarize(
    across(
...
descriptive_analysis_overview_table_robust <- data %>% 
  ...
  mutate(error_code = fct_relevel(error_code, "E-1", "E", "E+1", "other")) %>% 
  rename(
...
descriptive_location_plot_rt <- data %>% 
  ...
  mutate(distance = fct_relevel(distance, "-1", "0", "1")) %>% 
