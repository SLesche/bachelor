...
formula <- bf(
  ...
  rt | dec(decision) ~ 0 + rsi:error_factor:stimulus + (0 + rsi|p|id),
...
initfun_robust <- function() {# all pars in stancode need init here
  list(
    ...
    b_ndt = runif(tmp_dat_robust$K_ndt, 0.1, 0.15), # rep for each dim in ndt
...
initfun_classic <- function() {# all pars in stancode need init here
  list(
    ...
    b_ndt = runif(tmp_dat_classic$K_ndt, 0.1, 0.15), # rep for each dim in ndt
