...
Our behavioral findings show that errors lead to greater accuracy reduction in the short RSI condition compared to the long RSI condition, which aligns with earlier studies [@dudschig2009; @jentzsch2009; @buzzel2017]. Post-error performance was compromised particularly in the case of a short RSI, as evidenced by both increased response time and reduced accuracy. The long RSI condition exhibited less pronounced negative effects on both response time and accuracy. Even in the long RSI condition, reductions in accuracy post-error remained, indicating some lingering influence of maladaptive processes [@dudschig2009]. The stronger impact of errors in the short RSI condition on accuracy in the following trials indicates that maladaptive processes influence response behavior especially in the short RSI condition.

The number of omissions following error trials provides additional evidence for an increased role of maladaptive processes at short RSI. In the short condition, participants failed to respond at all in `r 100*proportion_omissions_location %>% filter(rsi == 0.2, location == "E+1") %>% pull(freq)`% of trials following an error compared to an average omission rate of `r 100*proportion_omissions_location %>% filter(rsi == 0.2, location != "E+1") %>% pull(freq) %>% mean(., na.rm = TRUE)`% in all other Go trials. In the long condition, this rate was reduced to `r 100*proportion_omissions_location %>% filter(rsi == 1, location == "E+1") %>% pull(freq)`%. The number of omissions may be accounted for by slower overall response times due to _error-monitoring processes_ taking up some amount of central cognitive resources [@dudschig2009]. This leads to a high rate of non-responses before the response deadline as the evidence accumulation process did not reach on of the response boundaries. Of course, omissions may also be the result of participants falsely responding to a Go trial by inhibiting their response. We attribute these omissions to participants failing to respond fast enough due to the large number of omissions, the shape of the response time distribution and the high number of omissions only occurring in the short RSI condition.
 
To further analyze the cognitive processes underlying post-error behavior we attempted to fit a Bayesian hierarchical DDM to data in both RSI conditions. However, we encountered problems when attempting to fit the model to data in the short RSI condition on post-error trials (E+1). These issues with fitting the data likely stem from the 800ms response time deadline. Participants responded slower in the short RSI condition and slower following errors. This results in a considerable proportion of responses (`r 100*proportion_omissions_location %>% filter(rsi == 0.2, location == "E+1") %>% pull(freq)`%) being coded as omissions because participants did not respond before the deadline. These omissions generate the steep drop after the 800ms deadline in this condition's response time distribution (see Figure \@ref(fig:discussion-ep1-short-plot-rt-dist)). The basic DDM is not able to account for this censoring of the response time distribution. Recently, researchers were able to extend the LBA model to account for omissions of this type [@damaso2021]. This can be accomplished by adding the number of omissions as a parameter that is estimated by an integral over the response time distribution between the deadline and infinity.
$$n_{omissions(ik)} = \int_{deadline}^{\infty} Wiener(a_{(ijk)}, z_{(ijk)}, Ter_{(ijk)}, v_{(ijk)})dT_{ijk}$$
Extending the DDM in a similar manner may result a better fit to the present empirical data. It would also allow us to test our initial hypothesis by comparing parameter changes in long RSI conditions to changes in short RSI conditions. However, developing and testing this extension would exceed the scope of this body of work and will be deferred to future endeavors.

Due to the models inability to accurately fit the response time distribution on post-error trials in the short RSI condition, parameters estimated in this condition can not be interpreted. This prevents us from gaining any insight into differences in effects of errors on post-error behavior depending on the RSI using DDM. 
...
In line with previous research [@dutilh2013; @purcell2016; @schiffler2017] we found post-error decreases in drift rate, even in the long RSI condition. Maladaptive processes such as decreased attention as posed by the orientating account [@notebaert2009] or reduced availability of central cognitive processing capabilities posited by @dudschig2009 seem to influence post-error behavior even after 1000ms have passed since the response. We also observed decreases in non-decision time following errors, but Bayes Factors indicate that the observed data provide evidence in favor of the null hypothesis of no effect of errors on non-decision time.

<!-- In the non-hierarchical DDM we found reverse effects on non-decision time. This is likely due to the high reliance of non-decision time $T_{er}$ on the minimal response times in the estimated condition. For this reason, interpreting differences between conditions remains problematic [@singmann2018]. This inconsistency further strengthens the idea that completely pooling data should be avoided, as it may lead to misleading results. -->

In contrast to our initial hypothesis, we observed decreases in boundary separation following error trials. Numerous previous studies [@white2010; @schiffler2017; @white2010; @dutilh2012testing] observed increases in boundary separation following errors. These findings support the idea that errors lead to increases in caution. We failed to find any evidence for increases such in caution following errors.
...
### Conclusion
We observed significant differences in the impact of errors on post-error behavior depending on the length of the RSI. In short RSI conditions, participants responded significantly slower and less accurate following an error. These error effects were reduced in amplitude in the long RSI condition. Accuracy was less impaired and response times were only somewhat slower following errors when using the robust approach and slightly faster when employing the classic approach. These differences in post-error effects imply that maladaptive processes affecting post-error accuracy impact performance especially when the RSI is short. Due to issues with model fit in post-error trials in the short RSI condition, we only fit the DDM to data in the long RSI condition. In this long condition, the DDM revealed decreases in drift rate and decreases in boundary separation to be responsible for post-error changes in behavior. We believe decreases in drift rate reflect impaired attentional processes and decreases in boundary separation reflect post-error recklessness or effects of emphasis on response speed.

Future research should address the issues that lead to problematic model fit in this study. The response deadline should be increased to reduce the number of omissions and a task that unequivocally requires response-selection should be chosen. As we observed maladaptive impacts of errors even 1000ms following the error, an even longer RSI condition may prove helpful in determining the duration of maladaptive effects following errors. We also suggest evaluating PES in both the classical and robust approach, as neither one can account for both local and global performance shifts. Finally, our failure to find adaptive effects following error responses in this task should discourage further researchers from using PES as a direct measure of cognitive control.
