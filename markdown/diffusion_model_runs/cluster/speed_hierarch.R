...
## ----set-formula------------------------------------------------------------------------------
formula <- bf(
  rt | dec(decision) ~ 0 + rsi:error_factor:stimulus + (0 + error_factor:stimulus||id),
  bs ~ 0 + rsi:error_factor + (0 + error_factor||id),
  ndt ~ 0 + rsi:error_factor + (0 + error_factor||id),
  bias ~ 0 + rsi:error_factor:previous_stimulus + (0 + error_factor:previous_stimulus||id)
)
...
## ----saving-ddm-classic-----------------------------------------------------------------------
save(fit_wiener, file = paste0("./bachelor/models/hierarch_model_speeding.rda"),
     compress = "xz")
...
save(pred, file = "./bachelor/predictions/pred_hierarch_speeding.rda")