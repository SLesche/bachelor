...
run_ddm <- function(participant){
  ...
  if (nrow(part_data) == 0){
    msg = paste("No data for participant", participant)
    return(msg)
  }
  ...
  prior = c(
    ...
    # Non-decision time
    set_prior("normal(0.1, 0.1)", class = "b", dpar = "ndt", lb = 0, ub = round(min_rt, 3)),
  ...
  save(fit_wiener, file = paste0("./bachelor/models/id/id_", participant, "_fit.rdata"))
  # store output
  return(fit_wiener)
}

ids = as.numeric(as.character(unique(data$id)))
