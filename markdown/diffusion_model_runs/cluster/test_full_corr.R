...
formula <- bf(
  ...
  rt | dec(decision) ~ 0 + rsi:error_factor:stimulus + (0 + rsi:error_factor:stimulus|p|id),
  bs ~ 0 + rsi:error_factor + (0 + rsi:error_factor|p|id),
  ...
  ndt ~ 0 + rsi:error_factor + (0 + rsi:error_factor|p|id),
  bias ~ 0 + previous_stimulus:error_factor + (0 + previous_stimulus:error_factor|p|id) # no reason for bias to vary TODO: intercept for bias
...
## ----saving-ddm-classic-----------------------------------------------------------------------
save(fit_wiener_location, file = paste0("./bachelor/models/test_full_corr_", Sys.Date(), ".rda"),
     compress = "xz")
