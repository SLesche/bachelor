...
prior <- c(
 ...
 # bias
 set_prior("beta(1.3, 1.3)", class = "b", dpar = "bias", lb = 0, ub = 1)
)
```
...
initfun_robust <- function() {# all pars in stancode need init here
  list(
    ...
    b_ndt = runif(tmp_dat_robust$K_ndt, 0.001, 0.002),
    b_bias = runif(tmp_dat_robust$K_bias, 0, 1)
  )
}
...
initfun_classic <- function() {# all pars in stancode need init here
  list(
    ...
    b_ndt = runif(tmp_dat_classic$K_ndt, 0.001, 0.002),
    b_bis = runif(tmp_dat_robust$K_bias, 0, 1)
  )
}
```
...
save(fit_wiener_classic, file = paste0("./markdown/models/classic_fullinter_rsi_err_", Sys.Date(), ".rda"),
     compress = "xz")
```
